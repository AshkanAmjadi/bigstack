<input id="mainAvatar" class="mainImageInput hidden" type="text" name="{{$name}}">
<input id="profile" class="form-file w-full hidden" oninput="cropImage(this,({{$size}}))"
       type="file">
<div class="action flex items-center gap-2">
    @component('component.btn.btnD',['color'=>'teal' , 'class' => 'hidden cropBtn mb-4'])
        @slot('title')
            <p class="!m-0 inline">
                برش عکس
            </p>
...
@php
    $img = old($name, $subject ? imgUrlMaker2($subject,$name) : null);
    if ($img == null ){
        $img =  $subject ? imgUrlMaker2($subject,$name) : null;
    }

@endphp
<div class="imageWraper rounded-xl DLL max-w-full inline-block">
    <div>
        <img class="w-72" alt="">
    </div>
</div>
