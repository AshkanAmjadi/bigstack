@if($person = $subject->getRelations()[$relation])
    <div class="flex items-center gap-3 flex-wrap">
        ...
        <a href="{{route('profile',['username'=>$person->username])}}" class="avatar xl md:!w-10 md:!h-10">
        ...
        <div class="flex flex-col">
            ...
            <p class="inline-block text-sm">
                {{persianDateOld($subject->created_at)}} created by  {!!  $person->name ?  htmlAlink(route('profile',['username'=>$person->username]),$person->name)  :'user'!!}
            </p>
        </div>
    </div>
@else
