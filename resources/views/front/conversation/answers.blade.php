

@if($conversation->has_best)
    @if(!$conversation->firstAnswers->first()->best)
        <a id="toAnswers" class="flex text-lg mt-3 gap-3 font-bold items-center">
            <svg xmlns="http://www.w3.org/2000/svg" class=" icon-lg text-emerald-500" width="24" height="24" viewBox="0 0 24 24">
                <g fill="none" stroke="currentColor" stroke-width="2.5">
                    <path d="M2 12c0-4.714 0-7.071 1.464-8.536C4.93 2 7.286 2 12 2c4.714 0 7.071 0 8.535 1.464C22 4.93 22 7.286 22 12c0 4.714 0 7.071-1.465 8.535C19.072 22 16.714 22 12 22s-7.071 0-8.536-1.465C2 19.072 2 16.714 2 12Z" opacity=".5"></path>
                    <path stroke-linecap="round" stroke-linejoin="round" d="m8.5 12.5l2 2l5-5"></path>
                </g>
            </svg>

            best
        </a>
        @include('component.answer.answer',['answer' =>$conversation->best_answer,'conversation' => $conversation])
    @endif
@endif

<a id="toAnswers" class="flex text-lg mt-3 gap-3 font-bold items-center">
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" class="icon-lg text-indigo-500" viewBox="0 0 24 24"><path fill="currentColor" d="m12.984 22.495l.537-.907c.416-.703.625-1.055.96-1.25c.334-.194.755-.201 1.598-.216c1.243-.021 2.023-.097 2.678-.368a4.952 4.952 0 0 0 2.68-2.68c.186-.446.28-.951.328-1.623c.025-.355.038-.533-.057-.675c-.095-.143-.275-.203-.636-.324c-1.511-.507-5.014-1.796-6.972-3.451c-2.207-1.867-4.182-5.66-4.889-7.115c-.14-.289-.21-.433-.334-.51c-.123-.076-.28-.074-.592-.071c-2.035.021-2.956.134-3.92.724A4.952 4.952 0 0 0 2.73 5.663C2 6.853 2 8.474 2 11.715v.99c0 2.307 0 3.46.377 4.37a4.952 4.952 0 0 0 2.681 2.679c.654.27 1.434.347 2.678.368c.842.015 1.264.022 1.598.216c.335.195.543.547.96 1.25l.537.907c.478.808 1.674.808 2.153 0Z" opacity=".5"></path><path fill="currentColor" fill-rule="evenodd" d="M14.872.24a.766.766 0 0 1-.008 1.137l-1.102 1.014c.959.009 1.881.03 2.714.083c.715.045 1.386.114 1.97.222c.572.106 1.123.26 1.56.507a5.837 5.837 0 0 1 2 1.839c.48.721.693 1.537.794 2.52c.1.963.1 2.166.1 3.691v.042c0 .445-.387.805-.864.805c-.478 0-.865-.36-.865-.805c0-1.576 0-2.702-.091-3.578c-.09-.864-.26-1.402-.543-1.827a4.16 4.16 0 0 0-1.425-1.31c-.186-.105-.509-.214-1.004-.305a15.098 15.098 0 0 0-1.75-.195A49.94 49.94 0 0 0 13.776 4l1.088 1c.34.313.343.822.008 1.139a.91.91 0 0 1-1.222.007L11.057 3.76a.778.778 0 0 1-.257-.572c0-.215.092-.421.257-.573L13.65.232a.91.91 0 0 1 1.222.007Z" clip-rule="evenodd"></path></svg>

        answers
    </a>

    @livewire('conversation.answers',['conversation'=>$conversation],key(''.now().''))

@if(!$toAnswer and $toAnswerObj)
    <div class="otherPage">
        <p class="forDelete text-mid text-rose-500 font-bold text-center">created to next pages.</p>
        @include('component.answer.answer',['answer' =>$toAnswerObj,'conversation' => $conversation])
    </div>
@endif

