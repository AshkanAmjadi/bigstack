...
@if($answer->toAnswer)
    <p id="{{$answer->hash_id}}" class="forDelete text-extr text-center">
        👇
    </p>
@endif
<article id="answer_section_{{$answer->hash_id}}"
         class="answer card_c p-6 md:p-4 @if($answer->best) ring-2 ring-emerald-500 @endif @if(!$answer->active) border-4 border-rose-500 @endif">
    <div class="flexBC gap-3">
        @include('front.components.who.answer' , ['relation' => 'user','subject' => $answer])
        <div class="flexC gap-3">
            @livewire('markable.best-answer',['obj'=>$answer,'conversation'=>$conversation],key(''.now().''))
            @if(!$answer->active)
    ...
    <div class="card_cwc p-5 md:p-3 mt-5">
        @include('component.description.description',['desc'=>editorJsDecode($answer,'content'),'altImage'=>$conversation->title])


    </div>
    @if($answer->mention)
        <div class="flex flex-wrap mt-3 gap-3">
            <div class="card_cwc text-rose-500 font-bold text-nsmid px-4 py-2">
                mentioned 👈
            </div>
            @foreach($answer->mention as $menstion)
                <a href="{{route('profile',['username'=>$menstion])}}" dir="ltr" class="card_cwc px-4 py-2 font-bold text-nsmid tracking-wider">
                    @ {{$menstion}}
                </a>
            @endforeach
        </div>
    ...
    <div class="w-full mt-5 flexC gap-2">
        @component('component.btn.btnD',['title' => 'Answer' , 'color'=>'indigo'])
            @slot('action')
        ...
        @if($answer->user_id == auth()->id() and !$answer->active or super())
            @component('component.btn.btnD',['title' => 'edit' , 'color'=>'emerald'])
                @slot('action')
                    onclick="AddAnswer('edit','{{$answer->hash_id}}')"
                @endslot
                @slot('icon')
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                        <g fill="currentColor">
                ...
                @endslot
            @endcomponent
            @livewire('conversation.delete',['id'=> $answer->id,'type'=>'Answer','name'=>'answer'],key(''.now().''))

        @endif
        @if(super())
            @livewire('admin.active',['obj' => $answer , 'size'=>'lg'],key(''.now().''))
        @endif
    </div>
    @component('component.divider.divider',['class' => 'mt-6'])

    @endcomponent
    <div class="tact">

        @livewire('markable.answer-tact',['obj'=>$answer],key(''.now().''))
    </div>
</article>
