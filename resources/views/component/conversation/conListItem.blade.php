<article class="w-full grid grid-cols-1 card_c p-2.5 @if($subject->private) ring-4 ring-sky-500 @endif @if(!$subject->active) border-4 border-rose-500 @endif "
         wire:key="con{{$subject->id}}">
        ...
        <div class="flex items-center gap-5 flex-wrap ">
            ...
            <div class="avatar 3xl md:!w-16 md:!h-16 -mt-12">
                <img class="pointer-events-none"
                     src="{{$subject->user->avatar ? imgUrlMaker2($subject->user,'avatar') : asset('assets/img/default.png')}}"
                     alt="avatar">
            ...
            <div class="flex flex-col">
                ...
                <p class="inline-block text-lg font-bold">
                    {{$subject->user->name ? :'no name'}}
                    @if(iAmInMention($subject))
                        <span class="text-sm mx-5" style="color: var(--accent-color)">
                        mentioned you in this question 👇
                    </span>
                    @endif
                </p>
                <p class="inline-block text-nsm font-semibold opacity-80">
                    {{persianDateOld($subject->created_at)}}
                    created by {!!  $subject->user->name ?  htmlAlink(route('profile',['username'=>$subject->user->username]),$subject->user->name)  :'user'!!}
    ...
    <div class="left px-2 space-y-4 mb-3">
        ...
        <div class="w-full card_cwc p-2.5">
            <div class="w-full card_cwc text-smid font-semibold line-clamp-3">
        ...
        <div class="w-full mt-5 flexC gap-2">
            ...
            @if($subject->user_id == auth()->id() and !$subject->active or super())
                @component('component.btn.linkBtn',['title' => 'edit' , 'color'=>'emerald','href'=> route('conversation.cratf.edit',[$subject->hash_id])])
                    ...
                    @endslot
                @endcomponent
                @livewire('conversation.delete',['id'=> $subject->id,'type'=>'Conversation','name'=>''],key(''.now().''))


            @endif
            @if(super())
                @livewire('admin.active',['obj' => $subject , 'size'=>'lg'],key($subject->getTable() .'_con_' . $subject->id))
            @endif
        </div>
    ...
    <div class="marks flex flex-row-reverse self-end ">
        ...
        @component('front.components.icon.numIcon',['number' => $subject->active_answers_count,'color'=>'indigo','text' => 'answers','iconSize'=>'sm'])
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
        ...
        @if($subject->has_best)
            ...
            @component('front.components.icon.numIcon',['color'=>'emerald','number' => 'Solved','iconSize'=>'sm'])
            ...
            @endcomponent
        @endif
        @if(!$subject->active and  super() or !$subject->active and $subject->user_id == auth()->id())
            @include('component.divider.hDivider')
