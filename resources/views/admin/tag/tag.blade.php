            ...
            <div class="overflow-x-auto overflow-y-clip">
                <table class="w-full table-style-shadow-td min-w-[1100px]">
                    ...
                    <tr>
                        ...
                        <th class="text-right">به عنوان دسته بندی</th>
                    ...
                    @foreach($list as $subject)
                        ...
                        <tr>
                            ...
                            <td class="w-28 p-3 relative wraper">
                                @if($subject->img)
                                    <div class="deleteImg absolute top-full right-1/2 translate-x-1/2 -translate-y-3/4"
                                         onclick="deleteImage(this,{{$subject->id}},'img')">
                                        @component('component.btn.btnD',['title' => 'حذف','size' => 'fsm' , 'color' => 'rose' , 'shadow' => false , 'id' => 'delete_img'.$subject->id.'btn'])
                                        @endcomponent
                                        @component('component.btn.btnD',['size' => 'fsm' , 'color' => 'rose' , 'shadow' => false , 'id' => 'delete_img'.$subject->id.'loader' , 'class' => 'hidden','iconspin' => true])
                                            @slot('icon')
                                ...
                                @endif
                                <div class="border-4 dark:border-slate-700 rounded-lg overflow-hidden flex justify-center">
                                    @if($subject->img)
                            ...
                            <td class="w-40 p-3 wraper relative">
                                @if($subject->banner)
                                    <div class="deleteImg absolute top-full right-1/2 translate-x-1/2 -translate-y-3/4"
                            ...
                            <td class="w-32 p-3 wraper relative">
                            ...
                            <td>
                                ...
                                @livewire('admin.active',['obj' => $subject , 'size'=>'lg' ,'subject' => 'searchable','color' => 'emerald','checkedIcon' => 'search'],key(''.now().''))
...
@section('footerScripts')
    ...
    <script>
        ...
        let deleteImg
        function deleteImage(El, id, type,subject = 'tag') {
            ...
            deleteImg = setTimeout(function () {
                ajaxReq(`{{url('/')}}/admin/${subject}/${id}/deleteImg`,
                    {type: type}
                    , function (data) {
                        console.log(data)
