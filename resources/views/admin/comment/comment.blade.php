...
@section('content')
                ...
                <table class="w-full table-style-shadow-td min-w-[1000px]">
                    ...
                    @foreach($list as $subject)
                        ...
                        <tr>
                            ...
                            <td class="relative">
                                @if($subject->new)
                                    @component('component.badg.badg',['title' => 'جدید','color'=>'rose','class'=>'absolute bottom-full-smaler-4 -right-1','shadow'=>false])

                                    @endcomponent
                                @endif

                                @include('user::admin.component.who' , ['relation' => 'user'])

                            </td>
                            <td>
                                <h2 class="font-bold text-smid">{{$subject->title}}</h2>
                                <div class="flex flex-wrap gap-2">
                                    @php($commentable = $subject->commentable)
                                    @php($table = $commentable->getTable())
                                    @php($name_fa = \App\facade\BaseQuery\BaseQuery::getPersionOfTable($table))
                                    @if($table === 'article')
                                        @php($nameOfsubject = $commentable->page_title)
                                    @else
                                        @php($nameOfsubject = $commentable->page_title)
                                    @endif
                                    @component('component.btn.linkBtn' ,['size'=>'fsm','color' => 'sky' , 'href' => route("$table.show",[$table => $commentable->slug])])
                                        @slot('title')
                                    {{$name_fa}} : {{$nameOfsubject}}
                                        @endslot
                                        @slot('icon')
                                            <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32"
                                                ...
                                                <path opacity=".8" fill="currentColor"
                            ...
                            <td class="max-w-[300px]">
                                <p class="text-nsm font-light w-full line-clamp-3">
                                    {{$subject->content}}
                                </p>
                            </td>
                            ...
                            <td>
                                <div class="flex flex-wrap gap-2 items-center">
                                    ...
                                    @component('component.btn.btnD',['color'=>'indigo','tabindex'=>true])
                                        @slot('action')
                                            onclick="commentPreview({{$subject->id}})"
                                        @endslot
                                        @slot('title')
                                            پیش نمایش نظر
                                        @endslot
    ...
    @component('admin.components.sidebar',['name' => 'comment'])
...
@section('footerScripts')
    ...
    <script>
        ...
        function closeAndActiveComment(id) {
            sidebar('hide', 'comment')
            selector(`label[for=activeSwitch${id}_comments]`).click()
        }

        function closeAndDeleteComment(id) {
            sidebar('hide', 'comment')
            selector(`.btn#Delete${id}`).click()
        }
