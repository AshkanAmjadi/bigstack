...
@php
    ...
    $subject = isset($possible) ? $possible : null;
    $update = [];
    $action = 'store';
        $prefix = 'admin.possible.';
        $name_en = 'possible';
        $name_fa = 'امکان';
        $name_fa_fard = 'امکانات';
    ...
    <div id="content" class="mini w-full pt-16 text-slate-700 pb-6">
        ...
        <div id="contentWraper" class="w-11/12 mx-auto mt-16 lg:w-96p sm:w-full space-y-6">
            ...
            <form id="addTag"  action="{{route("$prefix$action",$update)}}"
                ...
                <div class="wraper">
                    <h2 class="text-sm mr-4 font-semibold">آدرس توضیحات</h2>
                    <input class="form-input text-smid w-full" name="info_page" dir="ltr" value="{{old('info_page',$subject ? $subject->info_page : null)}}" type="url" placeholder="https://webmooz.com">
                </div>
                @error('info_page')
                @component('component.allert.allert' )
                    @slot('title')
