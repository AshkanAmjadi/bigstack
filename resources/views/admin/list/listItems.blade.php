@forelse($nav as $l1)
    ...
    <div data-listId="{{$l1->id}}" class="sortItem card_c p-3">
        ...
        <div class="flex items-center justify-between gap-2 flex-wrap" style="">
            ...
            <div class="flex text-smid flex-wrap lg:gap-5 items-center ">
                {{--                @if($l1->dark_icon)--}}
                {{--                    <div class="icon  ml-2 dark:!inline-block hidden">--}}
                {{--                        <img class="icon !w-6 !h-6" src="{{imgUrlMaker2($l1,'dark_icon')}}"--}}
                {{--                             alt="">--}}
                {{--                    </div>--}}
                {{--                @endif--}}
                {{--                @if($l1->icon)--}}

                {{--                    <div class="icon  ml-2 dark:hidden">--}}
                {{--                        <img class="icon !w-6 !h-6" src="{{imgUrlMaker2($l1,'icon')}}"--}}
                {{--                             alt="">--}}
                {{--                    </div>--}}
                {{--                @else--}}
                {{--                    todo pisfarz icon option--}}
                <div class="icon inline handler p-3 ">
                ...
                <p>
                    {{$l1->name}}
                    @if($listable = $l1->listable)
                        -
                        @if($listable->getTable() == 'category')
                            دسته
                            ({{$listable->title}})
                        @endif
                        @if($listable->getTable() == 'page')
                            صفحه
                            ({{$listable->title}})
                        @endif
                    @endif
                ...
                @livewire('admin.list.toggle-header',[$l1])
                @if($level < 2)
                    @livewire('admin.list.toggle-footer',[$l1])

                @endif
                @if($l1->parent_id == 0)
                    @livewire('admin.list.menu-type',[$l1])
                @endif
            ...
            <div class="left flex gap-2">
                ...
                @if($level <= 1)
                    ...
                    @component('component.btn.linkBtn',['color'=>'sky'])
                        @slot('href')
                            {{route($prefix.'index',['parent_id'=>$l1->id])}}
                        @endslot
                        @slot('title')
                            نمایش فرزندان
                        @endslot
                ...
                <form class="delete" action="{{ route($prefix.'destroy',[$name_en=>$l1->id]) }}"
                      method="post">
                    @csrf
                    @method('DELETE')
                    @component('component.btn.btnD',['color'=>'red','tabindex' => true])
                        ...
                        @component('component.dropdown.dropdown',['title' => 'توجه!!'])
                            ...
                            @slot('btns')
                                ...
                                @component('component.btn.btnD',['title' => 'حذف' , 'color' => 'rose' ,'size' => 'sm','id'=>'Delete'.$l1->id,'shadow'=>false])
                                    @slot('action')
...
@empty
    ...
    <div class="divider relative ">
        <div class="title absolute w-full text-sm font-medium justify-center flex -top-2">
            <p class="bg-slate-100 text-slate-400 px-2">فرزندی موجود نیست</p>
        </div>
    </div>
