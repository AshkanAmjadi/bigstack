<div>
    <div onclick="document.querySelector('#deleteCon{{$id}}{{$type}}').classList.toggle('noVisible')" class="relative">
        @component('component.btn.btnD',['title' => 'delete '.$name , 'color'=>'rose'])
            @slot('icon')
            ...
            @endslot
        @endcomponent
        <div id="deleteCon{{$id}}{{$type}}" class="absolute top-1/2 right-1/2 translate-y-1/2- translate-x-1/2 w-full noVisible" wire:click.debounce.200ms="delete">
            @component('component.btn.btnD',['title' => 'sure!!' , 'color'=>'red' ,'class' => 'w-full'])
                @slot('icon')
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="2 2 22 22">
                        <g fill="currentColor">
                            <path
