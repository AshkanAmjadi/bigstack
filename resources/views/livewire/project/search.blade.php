<div class="space-y-5">
        ...
        <div class="w-full space-y-6">
            <div class="relative">
                <input id="searchConversation" wire:input.debounce.1000ms="setSearch($event.target.value)"
                       ...
                       placeholder="Search ...">
                <label for="searchConversation" class="card_cwc p-3 md:p-2 absolute left-3 top-1/2 translate-y-1/2-">
    ...
    <div class="bottom flex gap-5 lg:gap-3 lg:flex-wrap lg:flex-col-reverse">
        <div class="w-1/4 lg:w-full flex flex-col">

            <div class="services space-y-3 mb-3 relative lg:hidden">
                @foreach($services as $srvc)
                    <div
                        class="glass rounded-lg cursor-pointer flex items-center p-4 gap-3 @if($srvc->name === $service) border-blue-500 !bg-blue-500/20 @endif "
                        wire:click="setService('{{$srvc->name}}')">
                        <img class="w-8" src="{{semanticImgUrlMaker($srvc,'img')}}" alt="{{$srvc->purpose}}">
                        <div class="p-normal text-smid font-bold  ">{{$srvc->name}}</div>
                        @if(!$srvc->active)
                            <div class="text-fsm opacity-75 font-bold">(in order)</div>
                        @endif
                    </div>
                @endforeach
            ...
            <div class="filters w-full lg:fixed lg:left-0 lg:p-5 lg:top-0 lg:z-50 @if(mobileDevice()) hide @endif">
        ...
        <div class="w-3/4 lg:w-full space-y-6">
            ...
            <div class="services space-y-3 mb-3 relative lg-r:hidden">
                @foreach($services as $srvc)
                    <div
                        class="glass rounded-lg cursor-pointer flex items-center p-4 gap-3 @if($srvc->name === $service) border-blue-500 !bg-blue-500/20 @endif "
                        wire:click="setService('{{$srvc->name}}')">
                ...
                @endforeach
                @include('component.backs.coloredOverlay')
            </div>
            ...
            <div class="grid grid-cols-3 1.5xl:gap-x-3 gap-7 xl:grid-cols-2 md:grid-cols-1 mt-6">
